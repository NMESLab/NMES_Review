...
#### Using SAE:
```{r}
SAE_rms_oneCycleCurrent <- sqrt(sum(oneCycleCurrent$currentWave^2) / (cycleDuration*samplingRate))
...
sqrt( ((2*pulseWidth_PW*i_amp)^2) / (cycleDuration) )
...
### Surface area
Finally, the RMS per cycle can be simply divided by the electrode area, with either of the values
```{r}
PWM_dutyCycleRMSCurrent_per_area <- PWM_dutyCycleRMSCurrent / electrodeArea
PWM_dutyCycleRMSCurrent_per_area

SAE_rms_oneCycleCurrent_per_area <- SAE_rms_oneCycleCurrent / electrodeArea
SAE_rms_oneCycleCurrent_per_area
```

This is the power per cm2. When expressing this in Watts per m^2, it is referred-to as *Surface Power Density*
```{r}
electrodeArea_m2 <- electrodeArea / (100^2)
SAE_rms_oneCycleCurrent_per_area_m2 <- SAE_rms_oneCycleCurrent / electrodeArea_m2
SAE_rms_oneCycleCurrent_per_area_m2
```


### Note regarding safety and the whole pulse train duration
The above calculations were made only per one cycle of the signal. 
If more than one pulse injected to the participant's, the RMS for the whole pulse train should be calculated.
If the pulse shape does not change from one cycle to the other, the RMS remains constant during the whole stimulation:

```{r}
pulseTrain_TotalRMS <- sqrt(mean(currentWave_df$currentWave^2))
pulseTrain_TotalRMS
```
...
phaseDuration_tau <- 500 * 1e-6 # in s
...
In their paper description they say that the answers should be 22.3 mA and 14.1 mA respectively, which coincides with the calculation above.


####*The section below needs to be checked according to O'Connor's email answer*

Now testing the same formula with the parameters used for the SAE and the PWM calculations:
```{r}
# With the parameters above
currentAmplitude_I <- 18 # in ms
pulseFrequency_f <- 1 / (20 * 1e-03) # in Hz
phaseDuration_tau <- 20 * 1e-03 # in s
currentRMS_I <- currentAmplitude_I * sqrt(2*pulseFrequency_f*phaseDuration_tau)
currentRMS_I
```

The result seems higher, but since they do not consider an off period in between each biphasic pulse, the RMS might be overshooting.